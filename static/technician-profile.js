...
const technicianIdText = document.getElementById("technicianIdText");
...
async function loadTechnicianProfile() {
    try {
        const technicianId = sessionStorage.getItem("technicianId") || new URLSearchParams(window.location.search).get("technician_id");
        
        if (!technicianId) {
            console.error("No technician ID found");
            showMessage("Technician ID not found. Please login again.", "error");
        ...
        if (nameDisplayEl) nameDisplayEl.textContent = data.name || data.technician_id || "Technician";
        
        // Display Technician ID
        if (technicianIdText) technicianIdText.textContent = data.technician_id || "N/A";
        
        // Display Area Assignment
        ...
        if (contactEl) {
            const contactValue = data.contact_number || data.mobile || "";
            contactEl.value = contactValue ? formatContact(contactValue) : "";
        }

        // Store technician ID if not already in sessionStorage
        if (data.technician_id && !sessionStorage.getItem("technicianId")) {
...
async function updateProfile() {
    ...
    if (email && email !== originalValues.email) {
        try {
            const tabId = getTabId();
            const checkRes = await fetch(`/api/technician/check-email?email=${encodeURIComponent(email)}&tab_id=${tabId}`);
    ...
    try {
        const tabId = getTabId();
        const res = await fetch(`/api/technician/update-profile?tab_id=${tabId}`, {
            method: "POST",
