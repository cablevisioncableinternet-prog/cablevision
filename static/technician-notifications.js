...
class TechnicianNotificationSystem {
    constructor() {
    ...
    init() {
        ...
        console.log("Initializing Technician Notification System...");
    ...
    async openAllNotificationsModal() {
        // I-refresh muna para siguradong pinaka-bago (parehong team-based
        // na fetch dahil galing pa rin sa technician_id ng naka-login)
    ...
    // ✅ RENDER FULL LIST NG NOTIFICATIONS SA MODAL
    ...
    getTechnicianIdentifier() {
        return localStorage.getItem('technicianId') || sessionStorage.getItem('technicianId');
    }
    ...
    async fetchNotifications() {
        ...
        try {
            const response = await fetch(`/api/technician/notifications?technician_id=${encodeURIComponent(technicianId)}`);
    ...
    async markAsRead(notificationId) {
        ...
        try {
            const response = await fetch(`/api/technician/notifications/${notificationId}/read?technician_id=${encodeURIComponent(technicianId)}`, { 
                method: "PATCH" 
            });
            if (response.ok) {
    ...
    async markAllAsRead() {
        ...
        try {
            const response = await fetch(`/api/technician/notifications/read-all?technician_id=${encodeURIComponent(technicianId)}`, { 
                method: "PUT" 
            });
            if (response.ok) {
    ...
    showToast(message, type = 'success') {
        let toast = document.getElementById("technicianToast");
        if (!toast) {
            toast = document.createElement("div");
            toast.id = "technicianToast";
            toast.className = "technician-toast";
            ...
            style.textContent = `
                ...
                .technician-toast.error { background: #991b1b; }
                .technician-toast.warning { background: #e69600; }
                @keyframes slideIn {
        ...
        toast.className = `technician-toast ${type}`;
...
window.TechnicianNotificationSystem = new TechnicianNotificationSystem();
